...
if(isset($_POST["submit"])) {
    ...
    $sql = "UPDATE `games` SET `slide$onslide` = 'images/".$file_name."' WHERE `games`.`id` = '".$querytype."'";
  ...
  $gameitem = mysqli_query($conn, "SELECT * FROM games WHERE id='$querytype'") or die(mysqli_error($conn));
  $game = mysqli_fetch_assoc($gameitem);

  if ($game["creator_id"] == $_SESSION["id"]) {echo "";} else {echo "<center>This presentation does not belong to you. <a href='presentations.aspx'>Back?</a></center>"; die();};
