<?php include('include.php');
  ...
  $iphashq = mysqli_query($conn, "UPDATE `user_form` SET `ip` = '".$iphash."' WHERE `user_form`.`id` = '".$_USER['id']."';");
   ...
   header('location:banned.aspx');
    ...
    $_banuserQ = mysqli_query($conn, "SELECT * FROM user_form WHERE id='$userid'") or die(mysqli_error($conn));
  $_banuser = mysqli_fetch_assoc($_banuserQ);
...
<center>
    ...
    <select name="usertoban" type="text" class="Text" min="1">
      <option value="<?php echo htmlspecialchars($_banuser['name']); ?>"><?php echo htmlspecialchars($_banuser['name']); ?></option>
    </select><br><br>
...
value='Bribery'>Bribery</option>
      ...
      <br>
      <br>
    <input name="off" type="text" tabindex="1" class="Text" placeholder="Offensive Item (optional)"><br><br>
